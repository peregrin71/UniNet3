...
## 4. Core Criteria and Deferred Handling

Core criteria are satisfied only if all of the following hold:
...
Deferred handling applies when any core criterion is missing:
...
6. Independent-review check: remove any row without a realistic analysis path.
