...
1. Markdown math must use LaTeX delimiters:
   ...
   - Never place $...$ or $$...$$ inside tick marks/backticks.
...
4. Symbols must use explicit subscripts/superscripts (no plain-text fallback when avoidable).
5. Use one canonical symbol per concept (no silent renaming).
6. Regime labels must be explicit: QM, GR, SM, Cosmology.
...
13. Linear projection: $\Pi_v: \mathcal{H} \to \mathbb{C}^{p,q}$
